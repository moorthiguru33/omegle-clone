/**
 * TamilPSD — Admin Processor v6 (Standalone Admin Panel)
 * ════════════════════════════════════════════════════════
 * CHANGES FROM v5:
 *  ✓ CORS: Allows ADMIN_ORIGIN env var (for separate Netlify admin site)
 *  ✓ New actions: listGithubFiles, readGithubFile (already existed, kept)
 *  ✓ Cloudinary upload: uses form-data correctly
 *  ✓ VIP: supports vip-emails.txt (plain) AND data/vip.json (hashed)
 *  ✓ BG remover: server-side processing endpoint added
 *  ✓ All previous v5 fixes retained
 */
...
function nextGroqKey(clientIndex) {
  ...
  const idx = (typeof clientIndex === 'number' && clientIndex >= 0)
    ? clientIndex : _groqCallCount++;
  return keys[Math.floor(idx / GROQ_PER_KEY) % keys.length];
}

// ── CORS — supports separate admin domain ──────────────────────────────────
// Set ADMIN_ORIGIN in Netlify env vars to your admin panel's Netlify URL
// e.g. https://tamilpsd-admin.netlify.app
// If not set, falls back to main domain
function getCorsOrigin(event) {
  const adminOrigin = process.env.ADMIN_ORIGIN || 'https://www.tamilpsd.in';
  const origin = event.headers?.origin || '';
  // Allow if it matches admin origin, main domain, or localhost dev
...
const ok  = (d, ev)    => ({ statusCode: 200, headers: buildCors(ev||{}), body: JSON.stringify(d) });
const err = (m, ev, code=500) => ({ statusCode: code, headers: buildCors(ev||{}), body: JSON.stringify({ error: m }) });
...
function cleanKey(name) {
  ...
  s = s.replace(/(\s+copy\s*\d*)+\s*$/i, '').trim();
  s = s.replace(/[()[\]{}]/g, ' ');
...
exports.handler = async (event) => {
  if (event.httpMethod === 'OPTIONS') return { statusCode: 204, headers: buildCors(event), body: '' };
  if (event.httpMethod !== 'POST') return err('POST only', event, 405);

  let body;
  try { body = JSON.parse(event.body || '{}'); }
  catch (e) { return err('Invalid JSON', event, 400); }

  const token = body.adminToken || '';
  if (!verifyAdminToken(token)) {
    console.warn('[adminProcessor] Unauthorized');
    return err('Unauthorized', event, 401);
  }
  ...
  try {
    switch (action) {
      case 'mediafire':          return ok({ result: await fetchMediaFire(body.urls || '') }, event);
      case 'gdrive':             return ok({ result: await fetchGDrive(body.urls || '') }, event);
      case 'cloudinary':         return ok({ result: await fetchCloudinary() }, event);
      case 'readXlsxGithub':     return ok({ result: await readXlsxFromGitHub(body) }, event);
      case 'groqSeoVision':      return ok({ result: await groqSeoVision(body) }, event);
      case 'saveXlsx':           return ok({ result: await buildXlsx(body.existingRows || [], body.newRows || []) }, event);
      case 'excelToJson':        return ok({ result: await excelToJsonFromGitHub(body) }, event);
      case 'githubPush':         return ok({ result: await pushToGitHub(body) }, event);
      case 'uploadToCloudinary': return ok({ result: await uploadToCloudinary(body) }, event);
      case 'fillMissing':        return ok({ result: await fillMissingCells(body) }, event);
      case 'readGithubFile':     return ok({ result: await readGithubFile(body) }, event);
      case 'listGithubFiles':    return ok({ result: await listGithubFiles(body) }, event);
      case 'buildVipJson':       return ok({ result: await buildVipJson(body) }, event);
      default:                   return err(`Unknown action: ${action}`, event, 400);
    }
  } catch (e) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      for (const m2 of matches) {
        const fileId = m2[1], fileName = m2[2];
    ...
    } catch (e) { console.warn('[GDrive] Error:', e.message); }
  }
  return result;
}
...
async function fetchCloudinary() {
  ...
  console.log(`[Cloudinary] ${total} images, ${Object.keys(result).length} keys`);
  return result;
}

// ════════════════════════════════════════════════════════
// 3. READ XLSX FROM GITHUB
// ════════════════════════════════════════════════════════
async function readXlsxFromGitHub({ owner, repo, branch, token }) {
  ...
  if (res.status === 404) return { rows: [], xlsxSha: null };
...
CRITICAL: Every design is unique. Your title, description, and tags MUST be 100% unique to THIS specific image.
...
TAGS: exactly 28 unique lowercase tags, comma-separated`;
  ...
  const visionContent = [];
  if (imageUrl && isVisionModel) visionContent.push({ type: 'image_url', image_url: { url: imageUrl, detail: 'high' } });
...
RETURN ONLY VALID JSON — no markdown, no backticks:
{
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (res.status === 429) { const e = new Error('Groq quota/rate_limit reached'); e.quotaError = true; throw e; }
...
function parseSeoResponse(data, id) {
  ...
  const genericPat = [/^tamil psd/i, /^photoshop template/i, /^free psd template/i, /^psd template/i];
  ...
  if (tagArr.length < 20) {
    const extras = ['psd free download','tamil psd','tamilnadu','photoshop template','free psd','editable psd','300 dpi','cmyk','print ready','flex banner','layered psd','smart object','tamil graphic design','free download','tamilpsd'];
...
function descPadding(id, visionLabel) {
  ...
  return `This ${d} file from TamilPSD.in is built at 300 DPI resolution in CMYK color mode. Every layer is named and grouped logically. Smart Object layers are used for photo placeholders.\n\nTo customize: open in Adobe Photoshop CS6 or any later version. Double-click Smart Object frames to replace photos. Click text layers to update names, dates, phone numbers.\n\nCompatible with Adobe Photoshop CS6, CC 2018, CC 2019, CC 2020, CC 2021, CC 2022, CC 2023, and CC 2024. No third-party plugins required.`;
}
...
function fallbackSeo(id) {
  ...
  return {
    ...
    category: cat,
    tags: `tamil psd, free psd download, ${dtype.toLowerCase()}, photoshop template, editable psd, flex print, 300 dpi, cmyk, print ready, tamil graphic design, free banner psd, tamilnadu, tamilpsd, layered psd, smart object, free download, banner design, ${cat} psd, tamilpsd free, photoshop file, flex design, south india, tamil, editable template, free flex, psd file, graphic design`,
...
async function buildXlsx(existingRows, newRows) {
  ...
  ws['!cols'] = [{wch:22},{wch:65},{wch:55},{wch:18},{wch:65},{wch:80},{wch:20},{wch:10},{wch:12},{wch:14},{wch:24},{wch:32},{wch:65}];
...
async function excelToJsonFromGitHub({ rows, owner, repo, branch, token }) {
  ...
  const output = (rows || []).filter(r => r.id).map(r => ({
    id: String(r.id||'').trim(), link: String(r.download_url||'').trim(),
    title: String(r.title||'').trim(), category: String(r.category||'').trim().toLowerCase(),
    tags: String(r.tags||'').trim(), description: String(r.description||'').trim(),
    dimensions: String(r.dimensions||'').trim(), thumb: String(r.preview_url||'').trim(),
    dpi: String(r.dpi||'300 DPI').trim(), fileSize: String(r.file_size||'').trim(),
    colorMode: String(r.color_mode||'CMYK').trim(), software: String(r.software||'').trim(),
  ...
  if (token) {
    ...
    } catch (e) { console.warn('[excelToJson] existing read failed:', e.message); }
  }
  ...
  for (const row of output) {
    ...
    if (old) { const m = { ...old }; for (const k of Object.keys(row)) { if (String(row[k]||'').trim()) m[k] = row[k]; } merged.push(m); }
...
async function pushToGitHub({ owner, repo, branch, token, path: filePath, content, message, isBase64, sha: bodySha }) {
  ...
  const headers = {
    ...
    'Content-Type': 'application/json', 'User-Agent': 'TamilPSD-Admin',
  };
...
async function uploadToCloudinary({ imageDataUrl, publicId }) {
  if (!CLD_CLOUD || !CLD_KEY || !CLD_SEC) throw new Error('Missing Cloudinary env vars');
  ...
  const sig = crypto_mod.createHash('sha1').update(`public_id=${cleanId}&timestamp=${timestamp}${CLD_SEC}`).digest('hex');

  // Use URLSearchParams instead of form-data to avoid dependency issues
  const formBody = new URLSearchParams();
  formBody.append('file', `data:image/jpeg;base64,${base64Data}`);
  formBody.append('public_id', cleanId);
  formBody.append('timestamp', String(timestamp));
  formBody.append('api_key', CLD_KEY);
  formBody.append('signature', sig);

  const res = await nodeFetch(`https://api.cloudinary.com/v1_1/${CLD_CLOUD}/image/upload`, {
    method: 'POST',
...
async function fillMissingCells({ row, model, lang, minWords }) {
  ...
  return {
    row: { ...row, title: row.title||seo.title||'', category: row.category||seo.category||'', tags: row.tags||seo.tags||'', description: row.description||seo.description||'', software: row.software||seo.software||'', fonts: row.fonts||seo.fonts||'', _filled: true },
    filled: true,
  };
}
...
// ════════════════════════════════════════════════════════
// 12. BUILD VIP JSON (hashes emails)
// ════════════════════════════════════════════════════════
async function buildVipJson({ emails }) {
  if (!Array.isArray(emails)) throw new Error('emails array required');
  const hashed = emails
    .map(e => e.trim().toLowerCase())
    .filter(e => e.includes('@'))
    .map(e => crypto_mod.createHash('sha256').update(e).digest('hex'));
  return { v: hashed, emails_count: hashed.length };
}
